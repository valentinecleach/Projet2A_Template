...
def user_and_movie(db_connection):
    ...
    user = user_dao.get_user_by_id(
        1
    )  # Assurez-vous qu'un utilisateur avec cet ID existe
...
def test_insert_comment(comment_dao, user_and_movie):
    ...
    if user and movie:
        comment = Comment(
            user=user, movie=movie, comment="Great movie!", date="2024-11-21"
        )
...
def test_get_comment_by_id(comment_dao, user_and_movie):
    ...
    if user and movie:
        comment = Comment(
            user=user, movie=movie, comment="Amazing visuals!", date="2024-11-21"
        )
        comment_id = comment_dao.insert(comment)
        ...
        assert (
            fetched_comment.comment == "Amazing visuals!"
        ), "Le contenu du commentaire ne correspond pas."
...
def test_delete_comment(comment_dao, user_and_movie):
    ...
    if user and movie:
        comment = Comment(
            user=user, movie=movie, comment="To be deleted", date="2024-11-21"
        )
        comment_id = comment_dao.insert(comment)
...
def test_get_comments_for_movie(comment_dao, user_and_movie):
    ...
    if user and movie:
        ...
        comment1 = Comment(
            user=user, movie=movie, comment="Loved the story!", date="2024-11-21"
        )
        comment2 = Comment(
            user=user, movie=movie, comment="Great acting!", date="2024-11-22"
        )
        ...
        assert (
            len(comments) >= 2
        ), "Tous les commentaires pour le film n'ont pas été récupérés."
        assert any(
            c.comment == "Loved the story!" for c in comments
        ), "Le commentaire 'Loved the story!' est manquant."
        assert any(
            c.comment == "Great acting!" for c in comments
        ), "Le commentaire 'Great acting!' est manquant."
