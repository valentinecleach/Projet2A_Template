...
def test_update_user(mock_db, user_dao):
    ...
    # Assertions
    assert user_dao.get_user_by_id(1).name == "John Updated"
    assert user_dao.get_user_by_id(1).email == "updated_john@example.com"
...
def test_delete_user(mock_db, user_dao):
    ...
    assert user_dao.get_user_by_id(1) is None
