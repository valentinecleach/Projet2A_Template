...
def test_insert_follow():
    ...
    assert (
        id_user,
        id_user_followed,
    ) in mock_db_connection.data, "Follow relationship should be inserted"
    assert (
        mock_db_connection.data[(id_user, id_user_followed)] == datetime.now().date()
    ), "Follow date should match"
...
def test_get_all_user_followed():
    ...
    assert followed_users == [
        ...
        3,
        4,
    ], "The followed users list should contain users 2, 3, and 4"
...
def test_get_all_user_followed_no_followers():
    ...
    followed_users = user_follow_dao.get_all_user_followed(id_user)
    assert (
        followed_users is None
    ), "If the user follows no one, the result should be None"
...
def test_is_following_existing():
    mock_db_connection = MockDBConnectorFollowDAO()
    mock_db_connection.data[(1, 2)] = (
        datetime.now().date()
    )  # Simule une relation existante
    user_follow_dao = UserFollowDao(mock_db_connection)
    is_following = user_follow_dao.is_following(1, 2)
    assert (
        is_following
    ), "is_following devrait retourner True pour une relation existante"


def test_is_following_non_existing():
    mock_db_connection = MockDBConnectorFollowDAO()
    user_follow_dao = UserFollowDao(mock_db_connection)
    is_following = user_follow_dao.is_following(
        1, 3
    )  # Pas de relation (1, 3) dans les données mock
    assert (
        not is_following
    ), "is_following devrait retourner False pour une relation inexistante"
